...
; It must be called before any other ADPCM decoder functions are called.
		...
		XDEF	_ADPCMDecoder_InitTables
_ADPCMDecoder_InitTables
...
_ADPCMDecoder
		...
		move.l	d0,-(sp)
		bsr.s	_ADPCMDecoder_Init
		move.l	(sp)+,d0

		move.l	a2,a0
		bsr.s	_ADPCMDecoder_Decode
...
;   ptrs within the ADPCMDecoderState structure between calls to ADPCMDecoder_Decode.
;
; The input should be d0/2 bytes large.
; The output will be d0 bytes large.
;
...
; a0		Decoder state
		...
		XDEF	_ADPCMDecoder_Decode
_ADPCMDecoder_Decode
